...
public class ShowPatrollingAreaScript : MonoBehaviour
{
    ...
    {
        if (data.idleBoundaryNegX != 0 && data.idleBoundaryNegY != 0 && data.idleBoundaryPosX != 0 && data.idleBoundaryPosY != 0)
        {
            ...
            {
                centerpoint.x += Mathf.Abs(data.idleBoundaryPosX) - Mathf.Abs(data.idleBoundaryNegX);
            }
            else if (Mathf.Abs(data.idleBoundaryPosX) < Mathf.Abs(data.idleBoundaryNegX))
            {
                centerpoint.x -= Mathf.Abs(data.idleBoundaryNegX) - Mathf.Abs(data.idleBoundaryPosX);
            }

            if (Mathf.Abs(data.idleBoundaryPosY) > Mathf.Abs(data.idleBoundaryNegY))
            {
                centerpoint.y += Mathf.Abs(data.idleBoundaryPosY) - Mathf.Abs(data.idleBoundaryNegY);
            }
            else if (Mathf.Abs(data.idleBoundaryPosY) < Mathf.Abs(data.idleBoundaryNegY))
            {
                centerpoint.y -= Mathf.Abs(data.idleBoundaryNegY) - Mathf.Abs(data.idleBoundaryPosY);
            }
